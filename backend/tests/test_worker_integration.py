...
def test_variant_selection_and_async_render_through_worker(
    ...
    shot_id = create_shot(client, project_id, duration=0.4)
    generation_jobs = [
        submit_generation(client, shot_id, f"render-variant-{index}").json()["job_id"]
    ...
    assert attempt_job_ids == set(generation_jobs)

    first_selection = client.post(f"/shots/{shot_id}/variants/{variants[0]['id']}/select")
    second_selection = client.post(f"/shots/{shot_id}/variants/{variants[1]['id']}/select")
    assert first_selection.status_code == second_selection.status_code == 200
    selected_variants = client.get(f"/shots/{shot_id}/variants").json()
    ...
    shot = client.get(f"/projects/{project_id}/shots").json()[0]
    assert shot["selected_variant_id"] == variants[1]["id"]
    ...
    assert event_types.count("VARIANT_CREATED") == 2
    assert event_types.count("VARIANT_SELECTED") == 2
