...
export const api = {
  ...
  uploadTourFootage: (shotId: string, file: File, rightsMetadata: Record<string, unknown>, idempotencyKey: string) => { const data = new FormData(); data.append("file", file); data.append("rights_metadata", JSON.stringify(rightsMetadata)); data.append("idempotency_key", idempotencyKey); return request<{ job_id: string; status: string; deduplicated: boolean; asset_id: string }>(`/shots/${shotId}/footage`, { method: "POST", body: data }); },
