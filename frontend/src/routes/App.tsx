...
import { AssetsPage, NewProjectPage, OverviewPage, ProjectsPage, RenderPage, ReviewPage, ShotsPage, TourRenderPage, TourTimelinePage } from "./pages";
import { PlanPage } from "./plan";

const musicStages = [["", "Setup"], ["assets", "Assets"], ["plan", "Plan"], ["shots", "Shots"], ["review", "Review"], ["render", "Render"]];
const tourStages = [["", "Brief"], ["plan", "Plan"], ["shots", "Shots"], ["review", "Review"], ["timeline", "Timeline"], ["render", "Render"]];
...
function Workspace() {
  ...
  const stages = state.data.project_type === "TOUR_GUIDE" ? tourStages : musicStages;
  return <AppFrame project={state.data}><nav className={`stages ${state.data.project_type === "TOUR_GUIDE" ? "tour-stages" : ""}`} aria-label="Project stages">{stages.map(([path, label], i) => <NavLink key={label} end={!path} to={`/projects/${projectId}${path ? `/${path}` : ""}`}><span>{i + 1}</span>{label}</NavLink>)}</nav><Outlet context={{ project: state.data, refreshProject: state.refresh } satisfies WorkspaceContext}/></AppFrame>;
}
...
export function App() { return <Routes><Route path="/projects" element={<AppFrame><ProjectsPage/></AppFrame>}/><Route path="/projects/new" element={<AppFrame><NewProjectPage/></AppFrame>}/><Route path="/projects/:projectId" element={<Workspace/>}><Route index element={<OverviewPage/>}/><Route path="assets" element={<AssetsPage/>}/><Route path="plan" element={<PlanPage/>}/><Route path="shots" element={<ShotsPage/>}/><Route path="review" element={<ReviewPage/>}/><Route path="timeline" element={<TourTimelinePage/>}/><Route path="render" element={<RenderRoute/>}/></Route><Route path="*" element={<Navigate to="/projects" replace/>}/></Routes>; }
function RenderRoute() { const { project } = useWorkspace(); return project.project_type === "TOUR_GUIDE" ? <TourRenderPage/> : <RenderPage/>; }
